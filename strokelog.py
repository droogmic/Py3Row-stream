...
if __name__ == '__main__':
    ...
    while workout['state'] == 1:
        ...
        with open('overlay.txt', 'w') as fw:
            min_val = int(monitor['pace']//60)
            sec_val = int(monitor['pace'] - min_val*60)
            overlay_text = "{}:{:02d}".format(min_val, sec_val)
            fw.write(overlay_text)
            print(overlay_text)
